...
class SliceStateAction:
    """Slice state to first 8 dims and action to last 7 dims"""
    
    def __call__(self, batch):
        # state 取前 8 个
        if "observation.state" in batch:
            batch["observation.state"] = batch["observation.state"][..., :8]

        # action 取后 7 个
        if "action" in batch:
            batch["action"] = batch["action"][..., -7:]
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        model=pi0_config.Pi0Config(max_token_len=300,action_horizon=16),
        data=MyDataConfig(
